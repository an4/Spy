...
Section \ref{tb:timingattacks} presents some of the most important timing attacks and explains one of the first side-channel attacks discovered by Kocher\cite{kocher1996timing} against an implementation of the Rivest-Shamir-Adleman (RSA) \cite{rivest1978method} cryptosystem, which was using the \textit{Square and Multiply}\cite{menezes1996handbook} method to compute the modular exponentiation. In Section \ref{tb:cacheAttacks} I outline some of the earlies cache attacks and describe the PRIME+PROBE\cite{osvik2006cache} method, which is used in numerous cache attacks \cite{osvik2006cache} and \cite{oren2015spy}. Additionally, I describe how the cache memory works in order to better understand how the attack presented by Oren et al.\cite{oren2015spy} works. The last section of this chapter, Section \ref{tb:browserattacks}, describes some of the most important browser attacks and presents how various browser technologies, such as HTML and JavaScript, are implemented and how they can be used to obtain side channel information.
...
Not all algorithms used for implementing the modular exponentiation step in RSA or similar cryptosystems are vulnerable to this particular timing attack. In his paper \cite{kocher1996timing}, Kocher suggested the use of the Chinese Remainder Theorem \cite{menezes1996handbook} (CRT) method instead of the \textit{Square and Multiply} algorithm described in Algorithm \ref{alg:squareandmultiply}, since it was believed that the CRT does not leak information about the private exponent $d$.
...
The browser starts by downloading the external resource from the address provided in the \texttt{src} attribute or an alternative address if this is not available. The \texttt{<img>} element has a series of events associated with it, which fire at various times. One such event is the \texttt{load} event, which announces that the browser has finished downloading the resource and is ready to display it. 
Occasionally, the browser is unable to display the file as an image, due to the resource not being a valid image file or other error which might have happened during the download process. In these cases, the \textbf{error} event will be triggered on the Image object once the browser becomes aware that something went wrong \cite{berjon2014html}.
...
In this dissertation I analysed the efficiency of some of the most important side channel attacks targeting browser software. Additionally, I selected two recent attacks for evaluation: my aim was to analyse their practical feasibility in state-of-the-art web browsers. The first attack, discovered by Oren et al.\cite{oren2015spy} uses the state of the browser's cache memory in order to determine a user's browsing activity. The second attack, proposed by Van Goethem et al.\cite{van2015clock} uses timing information in order to determine the size of a hidden file.

I discovered that the attack proposed by Oren et al.\cite{oren2015spy} and described in Section \ref{orenetal} is no longer possible due to an update in the JavaScript High Resolution Time API which no longer provides time measurements accurate enough to differentiate between a cache miss and a cache hit.

In the short time between the release of the paper by Van Goethem et al.\cite{van2015clock} and the start of this project, there have been major changes in the browser software which altered the efficiency of their proposed measuring methods. I was able to verify that the methods proposed by Van Goethem et al.\cite{van2015clock} are more efficient than previous measuring methods. Additionally, I discovered that the Video method only works in Chrome, Application Cache is going to be removed from all the major browsers and is no longer possible to measure the load time of a random file using the script method. 

To conclude, browser attacks are an important category of side-channel attacks which pose an imminent threat to the privacy of online users. Side-channel information obtained through running browser attacks can be used to disclose private information about the user, such as their location, browsing history and in some cases their current browsing activity. One interesting fact about such attacks is that the user is unaware that the website they are accessing is collecting information about them. 
