...
One of the cryptosystems vulnerable to the timing attack discovered by Kocher is RSA \cite{rivest1978method}. RSA requires the computation of $m = c^d \, mod \, N$, where $N$ is the public key, $d$ is the secret key and $c$ can be found by an adversary. The attacker's goal in timing attacks is to find the value of $d$. The modular exponentiation in RSA can be implemented using various algorithms. Square and multiply described in Algorithm \ref{alg:squareandmultiply} is a famous method for computing the modular exponentiation step in RSA; however, it is also vulnerable to timing attack as Kocher demonstrated in his paper \cite{kocher1996timing}.

The square and multiply algorithm computes $c^d \, mod \, N$. The algorithm iterates to every bit of $d$, starting from the most significant bit down to the least significant bit. At the start of the algorithm, the $result$ is initialised to $1$. For every step of the iteration in the square and multiply method, the $result$ is being squared. If the value of the current bit of $d$, $d_i$ is one the algorithm makes one additional step and multiplies the $result$ by $c$. At the end of the $for$ loop the algorithm returns the value of $result$. 
...
Not all algorithm used for implementing the modular exponentiation step in RSA or similar cryptosystems are vulnerable to this particular timing attack. In his paper \cite{kocher1996timing}, Kocher suggested the use of Chinese Remainder Theorem (CRT) instead of the square and multiply algorithm or Montgomery exponentiation.
...
In 2003, Brumley et al. \cite{brumley2005remote} were the first to show that web servers are vulnerable to timing attacks by successfully attacking OpenSSL \cite{openssl}. They were also able to recover the secret exponent in implementations of RSA using the Chinese Remainder Theorem, which was previously believed to be secure. 
