...
The methods used by Van Goethem et al. \cite{van2015clock} to measure the loading time of a file from a cross-origin website require the files to be stored remotely. In order to test the attack, I set up a remote server, which contains randomly generated HTML files of different sizes ranging from 50kB to 1024kB. The files can be split into three overlapping groups:
\begin{itemize}
...
The attacks presented by Van Goethem et al. in \cite{van2015clock} are based on the assumption that an adversary can distinguish between files of different only having access to the files' load times. I decided to test if using the Video element to load an external resource provides better results than when the Image element is used. I build a website which measures the load time of files of various sizes using both the Image element and the Video element and compares the efficiency of the two methods.

The web application can be split into two sections:
\begin{itemize}
\item A part which provides a friendly user interface for generating graphs as it can be seen in Figure \ref{fig:videointerface}
\end{itemize}
