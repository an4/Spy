...
\subsection{Felten et al. - Browser Cache Attack}
...
\subsection{Jia et al. - Geo-location Attack}
...
\subsection{Bortz et al. - Timing Web Application}
% rewrite
...
\subsection{Van Goethem et al. - Timing Attacks in the Modern Web}
...
\subsubsection{Real-world attacks}
...
\caption{Creating an Eviction Set}
\label{evictionsetalg}
\end{algorithm}

The attack presented by Oren et al. is written entirely in JavaScript. This means that the attack affect a wide range of systems since it only requires a browser with JavaScript support to run. On the other hand, the choice of language makes implementing the attack more difficult. JavaScript has no notion of pointer so it is impossible to determine the virtual address of a variable. Additionally, JavaScript has severely restricted access to the system so the attacker has to use the JavaScript High Resolution API, which provides sub millisecond time measurements \cite{oren2015spy}.

% describe method for checking if the attack works
