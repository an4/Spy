...
In general, side-channel attacks record the power consumption, electromagnetic emission, heat dissipation,   execution time or the state of the cache memory while the device is performing an interesting operation and use it to determine the device's internal state which is related to the secret parameters involved in the computation.  This dissertation is about side-channel attacks in the browser, a very powerful category of side-channel attacks which affect browser software and whose aim is to disclose private data about the user through either using timing information (\textit{Timing Attacks}) or the state of the browser's cache memory (\textit{Cache Attacks}). 
\\\\
\noindent
The world of browser attacks is rapidly changing, with new vulnerabilities appearing and disappearing within weeks. 
\\\\
\noindent

\begin{itemize}
\item Reviewed some of the most important Side-Channel attacks targeting browser software.
\item Tested if the attack discovered bt Oren et al. \cite{oren2015spy} is still possible.
\item Implemented the attacks proposed by Van Goethem et al. \cite{van2015clock} and analyse their performance against previous attacks.
\end{itemize}
...
In 2006, Osvik et al. \cite{osvik2006cache} presented a cache attack against AES based on inter-process leakage. Their paper also describes the PRIME+PROBE algorithm in the context of the first level cache, L1, which was later extended to last level cache (LLC) by Liu et al. \cite{liu2015last}.
...
Clover believes that none of the proposed solutions is a valid countermeasure. He also thinks that the attack is possible due to a CSS implementation issue rather than being a browser related problem \cite{cssvisited}. The attack discovered by Clover is no longer possible. Browser vendors updated their software such that when an adversary requests the computed style, the browser lies and send back all links as unvisited.
