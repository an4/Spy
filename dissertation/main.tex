...
% rewrite
Another class of browser attacks was identified by Bortz et al. in \cite{bortz2007exposing}. Their attack allows an adversary to obtain information about the victim's view of another website. Imagine and adversary, Eve, wants to see Alice's view of Bob's website. Eve builds a website, www.eve.com, which when accessed sends a request to Bob's website, www.bob.com. Eve sends the link to her website to Alice. Bob's website can only display two messages, based on whether Bob knows the person who is accessing the website or not. When Alice accesses Eve's malicious website, Eve's website can receive from www.bob.com one of two responses either Alice and Bob are friends or they are not. The information that Eve gains access to should have only been available to Alice, therefore Eve's attack is exposing private data about the two parties. 
...
The attack uses the \texttt{<img>} element found in HTML to estimate the size of the data the victim has access to. The attacker loads the address of the target data into an \texttt{<img>} element and 
